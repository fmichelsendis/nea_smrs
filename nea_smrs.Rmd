---
title: "Overview of SMR related activities" 
#author: F. Michel-Sendis
output: html_document
---


```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE, message = FALSE, warning = FALSE)

library(tidyverse)
library(data.table)
library(DT)
library(plotly)

df<-fread("csv/nea_smrs_v2.csv", header = TRUE)%>%
  select(KEY, PROJECT, DELIVERABLE, DUE_DATE, 13:46)%>%
  pivot_longer(cols = c(16:38), names_to = "TOPIC", values_to = "FLAG")%>%
  mutate(FLAG=toupper(FLAG),
         SMR=as.factor(SMR))%>%
  filter(FLAG=="YES")%>%
  select(-FLAG)

df2<-df%>%
  pivot_longer(cols = c(5:13), names_to = "COMMITTEE", values_to = "FLAG")%>%
  mutate(FLAG=toupper(FLAG)
         )%>%
  filter(FLAG=="YES")

#projects only
df1<-df2%>%select(-TOPIC, -STATUS, -DUE_DATE, -DELIVERABLE, -FLAG)%>%unique()
#write_csv(df1, 'csv/df1.csv')

Nproject<-as.integer(dim(df1)[1])

Ncommittees<-as.integer(dim(as.data.frame(unique(df1$COMMITTEE)))[1])
                        
```

\vspace{2cm}

# SMR projects per committee

Total number of projects : `r Nproject` SMR related projects found on `r Ncommittees` committees.

Specific projects related to SMRS are found in 6 committees :

```{r histo1, echo=FALSE, fig.width=10, fight.height=70}
library(packcircles)

#df11<-fread("csv/df1.csv")
g<-ggplot(df1, aes(y=SMR, fill=SMR)) +
  geom_bar(stat='count', position = "dodge") +
  theme(legend.position = "top") +
  theme_light() +
  theme(legend.position = "none") +
  facet_wrap(vars(COMMITTEE)) +
  xlab("")+
  ylab("") 
 


df_b<-df1%>%select(KEY, SMR, COMMITTEE)%>%
  group_by(COMMITTEE, SMR)%>%
  count(COMMITTEE, SMR)

df_b_s<-df_b%>%filter(SMR=='Specific')
df_b_r<-df_b%>%filter(SMR=='Relevant')

packing <- circleProgressiveLayout(df_b_s$n, sizetype='area')
dat.gg <- circleLayoutVertices(packing, npoints=50)
data<-cbind(df_b_s, packing)

  
# Make the plot
g<-ggplot() + 
  geom_polygon(data = dat.gg, aes(x, y, group = id, fill=as.factor(id)), colour = "black", alpha = 0.6) +
  geom_text(data = data, aes(x, y, label = COMMITTEE))+
  scale_size_continuous(range = c(1,4)) +
  theme_void() + 
  theme(legend.position="none") +
  coord_equal()

g


```


# SMR topics addressed per committee

```{r histo2, echo=FALSE, fig.width=10, fight.height=70}
g<-ggplot(df2, aes(y=SMR,  fill=TOPIC)) +
  geom_bar(stat='count', position = "dodge") +
  theme(legend.position = "top") +
  theme_light() + 
  facet_wrap(vars(COMMITTEE)) +
  xlab("")+
  ylab("")  

ggplotly(g)

```

# SMR topics addressed per broad scope

```{r bubbles2, echo=FALSE, fig.width=8, fight.height=35}

dft<-fread("csv/topics_scopes.csv")%>%select(Scope, Count)

packing <- circleProgressiveLayout(dft$Count, sizetype='area')
dat.gg <- circleLayoutVertices(packing, npoints=50)
data<-cbind(dft, packing)

g<-ggplot() + 
  geom_polygon(data = dat.gg, aes(x, y, group = id, fill=as.factor(id)), colour = "black", alpha = 0.6) +
  geom_text(data = data, aes(x, y, label = Scope))+
  scale_size_continuous(range = c(1,4)) +
  theme_void() + 
  theme(legend.position="none") +
  coord_equal()

g


```

 


# Table

```{r data, echo=FALSE}


DT::datatable(df1)

```


